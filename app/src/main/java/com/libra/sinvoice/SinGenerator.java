...
public class SinGenerator {
	...
	private static final int STATE_START = 1;// 开始状态
	private static final int STATE_STOP = 2;// 结束状态

	public static final int BITS_8 = 128;// 8位最大值
	...
	public static final int UNIT_ACCURACY_1 = 4;// 单元精度
	...
	// 产生正弦波，传入正弦波频率，持续时间，比如产生10秒的正弦波
	public void gen(int genRate, int duration) {
		if (STATE_START == mState) {// 看状态开始。。
			mGenRate = genRate;
			mDuration = duration;
			...
			int n = mBits / 2;// 振幅最大值/2??
			int totalCount = (mDuration * mSampleRate) / 1000;// K为单位采样个数
			double per = (mGenRate / (double) mSampleRate) * 2 * Math.PI;// 一秒钟采样间隔时间
																			// 累积时间+per获得下一次采样横坐标
			double d = 0;// 已采样时间累计

			LogHelper.d(TAG, "genRate:" + genRate);
			if (null != mCallback) {
				mFilledSize = 0;// 已记录文件大小
				BufferData buffer = mCallback.getGenBuffer();// 获得存储内从空间
				if (null != buffer) {
					for (int i = 0; i < totalCount; ++i) {
						if (STATE_START == mState) {
							int out = (int) (Math.sin(d) * n) + 128;// sin（x）*振幅
																	// +
																	// 128（只出现正数？）
